...
function getConfig() {
  ...
  if ((!owner || !repo) && process.env.GITHUB_REPO) {
    const parts = String(process.env.GITHUB_REPO).trim().split("/").filter(Boolean);
    if (parts.length >= 2) {
      owner = parts[0];
      repo = parts[1];
    }
  }
  if ((!owner || !repo) && process.env.NEXUS_REPO) {
    const parts = String(process.env.NEXUS_REPO).trim().split("/").filter(Boolean);
    if (parts.length >= 2) {
      owner = parts[0];
      repo = parts[1];
    }
...
async function getDefaultBranch(projectId, userId) {
  ...
  const { data } = await client.get(`/repos/${config.owner}/${config.repo}`);
...
async function createBranch(baseBranch, newBranch, projectId, userId) {
  ...
  const client = createClient(config);
  const refRes = await client.get(`/repos/${config.owner}/${config.repo}/git/ref/heads/${baseBranch}`).catch(() => null);
...
async function createCommit(branch, message, files, projectId, userId) {
  ...
  if (!config.owner || !config.repo) throw new Error("GitHub repo no configurado");
  const client = createClient(config);
  const refRes = await client.get(`/repos/${config.owner}/${config.repo}/git/ref/heads/${branch}`).catch(() => null);
  if (!refRes || !refRes.data || !refRes.data.object || !refRes.data.object.sha) {
    throw new Error(`Rama ${branch} no encontrada`);
  }
  const baseSha = refRes.data.object.sha;
  const commitRes = await client.get(`/repos/${config.owner}/${config.repo}/git/commits/${baseSha}`);
  ...
  for (const f of files || []) {
    ...
    const blobRes = await client.post(`/repos/${config.owner}/${config.repo}/git/blobs`, {
  ...
  const treeRes = await client.post(`/repos/${config.owner}/${config.repo}/git/trees`, {
    base_tree: baseTreeSha,
    tree
  });
  const commitRes2 = await client.post(`/repos/${config.owner}/${config.repo}/git/commits`, {
    message,
...
async function createPullRequest(title, headBranch, baseBranch, body, projectId, userId) {
  ...
  try {
    const { data } = await client.post(`/repos/${config.owner}/${config.repo}/pulls`, {
    ...
    const ghMessage =
      ghData?.message ||
      (typeof ghData?.error === "string" ? ghData.error : null) ||
      err.message ||
      "Error al crear Pull Request en GitHub";

    const wrapped = new Error(ghMessage);
    wrapped.statusCode = statusCode;
    wrapped.code = err.code || wrapped.code;
    wrapped.details = ghData || null;
    throw wrapped;
  }
}
...
async function getBranches(projectId, userId) {
  ...
  if (!config.owner || !config.repo) throw new Error("GitHub repo no configurado");
  const client = createClient(config);
  const { data } = await client.get(`/repos/${config.owner}/${config.repo}/branches`, { params: { per_page: 100 } });
...
async function getPullRequests(state = "open", projectId, userId) {
  ...
  if (!config.owner || !config.repo) throw new Error("GitHub repo no configurado");
  const client = createClient(config);
  const { data } = await client.get(`/repos/${config.owner}/${config.repo}/pulls`, {
...
async function getPullRequestStatus(prNumber, projectId, userId) {
  ...
  if (!config.owner || !config.repo) throw new Error("GitHub repo no configurado");
  const client = createClient(config);
  const { data } = await client.get(`/repos/${config.owner}/${config.repo}/pulls/${prNumber}`);
  return {
    number: data.number,
    state: data.state,
    title: data.title,
...
async function getCommits(projectId, userId, limit = 20) {
  ...
  if (!config.owner || !config.repo) throw new Error("GitHub repo no configurado");
  const client = createClient(config);
  const { data } = await client.get(`/repos/${config.owner}/${config.repo}/commits`, {
...
async function getReleases(projectId, userId) {
  ...
  if (!config.owner || !config.repo) throw new Error("GitHub repo no configurado");
  const client = createClient(config);
  const { data } = await client.get(`/repos/${config.owner}/${config.repo}/releases`, {
    params: { per_page: 100 }
  });
  const list = Array.isArray(data) ? data : [];
  logger.info(
...
async function getRepositoryTags(projectId, userId) {
  ...
  if (!config.owner || !config.repo) throw new Error("GitHub repo no configurado");
  const client = createClient(config);
  const { data } = await client.get(`/repos/${config.owner}/${config.repo}/tags`, {
    params: { per_page: 100 }
  });
  const list = Array.isArray(data) ? data : [];
  logger.info(
...
async function getContributors(projectId, userId) {
  ...
  if (!config.owner || !config.repo) throw new Error("GitHub repo no configurado");
  const client = createClient(config);
  const { data } = await client.get(`/repos/${config.owner}/${config.repo}/contributors`, {
    params: { per_page: 100 }
  });
  const list = Array.isArray(data) ? data : [];
  return list.map((cc) => {
...
async function getFileContentByPath(path, ref, projectId, userId) {
  ...
  const res = await client
    .get(url, { params: ref ? { ref } : undefined })
    .catch((e) => {
