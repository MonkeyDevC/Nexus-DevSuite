...
const { getBaseUrl, DEFAULT_MASTER_LOGIN } = require("./support/constants.js");
const { seedSession } = require("./support/authSession.js");
const { ensureEmployeeTokens } = require("./support/employeeUser.js");

const BASE = getBaseUrl();
const LOGIN_BODY = DEFAULT_MASTER_LOGIN;
...
test.describe("SPA routing: single mount and real path navigation", () => {
  test("React shell mounts once per browser context and history navigation uses real paths", async ({
    browser,
    page,
  }, testInfo) => {
    const pageErrors = [];
...
test.describe("Backlog and work domains: navigation, empty states, deep links, and cross-route return", () => {
  test("backlog, features, stories, sprints, incidents, and releases surfaces respond with expected context and guards", async ({
    page,
    request,
  }, testInfo) => {
    const pageErrors = [];
    ...
    await page.goto(`${BASE}/backlog?project=${encodeURIComponent(primaryId)}`, { waitUntil: "domcontentloaded" });
    await expect(reactRoot).toBeVisible({ timeout: 30000 });
    ...
    const backlogStoryButtons = page
      .getByTestId("backlog-table")
      .locator('tbody tr button[data-testid^="story-backlog-row-"]');
    const backlogStoryCount = await backlogStoryButtons.count();
    if (backlogStoryCount > 0) {
      await backlogStoryButtons.first().click();
      await assertOneMount();
      ...
      const uStory = new URL(page.url());
      expect(uStory.pathname).toBe("/backlog");
      expect(uStory.searchParams.get("project")).toBe(primaryId);
      expect(uStory.searchParams.get("story")).toBeTruthy();
      await page.getByRole("button", { name: "Cerrar historia" }).click();
      await expect(page.getByTestId("backlog-root")).toBeVisible({ timeout: 30000 });
      expect(new URL(page.url()).searchParams.get("story")).toBeFalsy();
    } else {
      await expect(page.getByTestId("backlog-table")).toContainText("Sin historias en backlog");
    }

    // Ruta legacy: redirect SPA a canónico `/backlog?project=` (path pisa query si difieren).
    pageErrors.splice(0, pageErrors.length);
    await page.goto(`${BASE}/projects/${encodeURIComponent(primaryId)}/backlog`, { waitUntil: "domcontentloaded" });
    await expect(reactRoot).toBeVisible({ timeout: 30000 });
    await assertOneMount();
    {
      const u = new URL(page.url());
      expect(u.pathname).toBe("/backlog");
      expect(u.searchParams.get("project")).toBe(primaryId);
    }
    await expect(page.getByTestId("backlog-root")).toBeVisible({ timeout: 30000 });
    await expect(page.getByTestId("backlog-root")).toContainText(/Backlog de producto/i, { timeout: 30000 });
    await expect(page.getByTestId("product-backlog-toolbar")).toBeVisible({ timeout: 30000 });
    await expect(page.getByTestId("backlog-loading")).toHaveCount(0, { timeout: 60000 });
    expect(pageErrors, `pageerror tras redirect legacy backlog:\n${pageErrors.join("\n")}`).toEqual([]);

    if (storyCtx?.secondProjectId) {
      pageErrors.splice(0, pageErrors.length);
      await page.goto(
        `${BASE}/projects/${encodeURIComponent(primaryId)}/backlog?project=${encodeURIComponent(storyCtx.secondProjectId)}`,
        { waitUntil: "domcontentloaded" }
      );
      await assertOneMount();
      const u2 = new URL(page.url());
      expect(u2.pathname).toBe("/backlog");
      expect(u2.searchParams.get("project")).toBe(primaryId);
      expect(pageErrors, `pageerror redirect legacy backlog (path gana query):\n${pageErrors.join("\n")}`).toEqual([]);
    }
    ...
    await expect(page.getByRole("heading", { level: 1, name: /Features - Nexus DevSuite/ })).toBeVisible({
      timeout: 30000,
    });
    ...
    await assertOneMount();
    await expect(page.getByTestId("feature-detail-message")).toContainText("Elemento no encontrado", { timeout: 30000 });
    ...
    await page.goto(`${BASE}/sprints?project=${encodeURIComponent(sprintListPid)}`, { waitUntil: "domcontentloaded" });
    await assertOneMount();
    ...
    if (sprintCtx) {
      const sprintDetailPath = `/sprints/${sprintCtx.sprintId}`;
    ...
    await page.goto(`${BASE}/sprints/${FAKE_UUID_SPRINT}`, { waitUntil: "domcontentloaded" });
    await assertOneMount();
    ...
    if (sprintCtx && sprintCtx.secondProjectId) {
      const mismatchSprint = `/sprints/${sprintCtx.sprintId}?project=${encodeURIComponent(sprintCtx.secondProjectId)}`;
...
test.describe("Admin: RBAC for MASTER vs EMPLOYEE and deep-link reload", () => {
  test("MASTER sees admin surface; EMPLOYEE is redirected from /admin; deep link and reload preserve policy", async ({
    browser,
    request,
  }) => {
