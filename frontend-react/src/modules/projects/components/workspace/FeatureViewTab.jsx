...
/** Resumen derivado: el sprint vive en cada historia; la feature no tiene sprint propio en el modelo. */
...
export default function FeatureViewTab({ feature, stories = [] }) {
  ...
  const sprintMetaText =
    sprintSummary.total === 0
      ? "Sin datos de backlog"
      : sprintSummary.withSprint === 0
        ? `Ninguna en sprint (${sprintSummary.without})`
        : sprintSummary.sprintNames.length === 1
          ? `${sprintSummary.sprintNames[0]} (${sprintSummary.withSprint})`
          : `${sprintSummary.withSprint}/${sprintSummary.total} en sprint${
      ...
      <div className={styles.metaBarWrap}>
        ...
        <div id={moreDetailsPanelId} className={styles.metaDetailsPanel} hidden={!moreDetailsOpen}>
          <div className={styles.metaDetailsGrid}>
            ...
            <div className={styles.metaDetailCell}>
              <span className={styles.metaDetailKey}>Sprints en historias</span>
              <span className={styles.metaDetailVal}>
                {sprintSummary.total === 0
                  ? "—"
                  : sprintSummary.sprintNames.length > 0
                    ? sprintSummary.sprintNames.join(", ")
                    : "Ninguna historia en sprint"}
              </span>
            </div>
          </div>
        </div>
