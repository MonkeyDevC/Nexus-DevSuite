...
import { uploadProjectEvidenceImage } from "../../index.js";
 ...
 * @param {boolean} [props.previewOnly] — solo panel de vista previa, sin editor ni split ni conmutador de modo
 */
export default function EvidenceStudio({
  ...
  const [mode, setMode] = useState(() => (previewOnly ? "preview" : "split"));
  const [isFullscreen, setIsFullscreen] = useState(false);

  useEffect(() => {
    if (previewOnly) setMode("preview");
  }, [previewOnly, historyResetKey]);
  ...
  const formatDisabled = disabled || mode === "preview" || previewOnly;
  ...
  const previewPaneProps = {
    value,
    emphasizeHeadingHierarchy: previewOnly,
  };
  ...
  return (
    <div className={shellClass} data-testid="evidence-studio-root">
      ...
      <div className={styles.body}>
        {previewOnly ? (
          <div
            ...
            role="region"
            aria-label="Vista previa de evidencia"
            className={styles.tabPanel}
          >
            <EvidencePreviewPane {...previewPaneProps} />
          </div>
        ) : (
          <>
            <div
            ...
            >
              {mode === "preview" ? <EvidencePreviewPane {...previewPaneProps} /> : null}
            </div>
      ...
      <EvidenceStudioFooter readOnly={readOnlyFooter} previewOnly={previewOnly} />
    </div>
  );
}
