...
import { listSprints, startSprint } from "../../modules/sprints/sprintsService.js";
...
const SPRINT_LIST_PAGE_SIZE = 50;

/**
 * Lista todos los sprints del proyecto paginando con el límite máximo permitido por la API.
 * @param {string} projectId
 * @returns {Promise<Array>}
 */
async function loadAllSprintsForProject(projectId) {
  const all = [];
  let page = 1;
  while (true) {
    const { items, meta } = await listSprints(projectId, {
      page,
      limit: SPRINT_LIST_PAGE_SIZE,
    });
    const batch = Array.isArray(items) ? items : [];
    all.push(...batch);
    const totalPages = Number(meta?.totalPages);
    const isLastPage = Number.isFinite(totalPages) && totalPages >= 1
      ? page >= totalPages
      : batch.length < SPRINT_LIST_PAGE_SIZE;
    if (isLastPage) break;
    page += 1;
  }
  return all;
...
export default function ProjectEditTab({
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const items = await loadAllSprintsForProject(projectReadonlyMeta.id);
  ...
  function handleExportWorkspaceSlice() {
    const payload = {
      ...
      evidence_markdown: draftSlice.evidence_markdown,
        ...
        const patch = {
          ...
          evidence_markdown:
            typeof parsed.evidence_markdown === "string"
              ? parsed.evidence_markdown
              : draftSlice.evidence_markdown,
        };
