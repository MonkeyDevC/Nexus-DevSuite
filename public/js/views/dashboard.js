...
(function () {
  ...
  window.registerView("dashboard", async function () {
    ...
    var titleHtml = '<div class="dashboard-hero"><h1 class="nexus-page-title mb-1">Bienvenido, ' + escapeHtml(userName) + '</h1><p class="dashboard-hero-subtitle mb-0">Vista general del estado de Nexus DevSuite</p></div>';
    ...
    var projects = [];
    var storiesInProgress = [];
    var recentActivity = [];
    var summaryAvailable = false;

    var projectsPromise = window.fetchApi("/projects?page=1&limit=10").then(function (body) {
    ...
    var summaryPromise = window.fetchApi("/dashboard/summary")
      .then(function (body) {
        if (body && body.success && body.data) {
          summary = body.data;
    ...
    var storiesInProgressPromise = projectsPromise.then(function (projectList) {
      var items = Array.isArray(projectList) ? projectList.slice(0, 10) : [];
      if (!items.length) return [];
      return Promise.all(
        items.map(function (p) {
          var pid = p && p.id ? p.id : "";
          if (!pid) return Promise.resolve([]);
          var qBase = "/projects/" + pid + "/backlog?page=1&limit=50";
          return Promise.all([
            window.fetchApi(qBase + "&status=IN_PROGRESS"),
            window.fetchApi(qBase + "&status=IN_REVIEW")
          ]).then(function (responses) {
            var out = [];
            responses.forEach(function (res) {
              if (!(res && res.success && res.data)) return;
              var stories = Array.isArray(res.data.stories) ? res.data.stories : [];
              stories.forEach(function (st) {
                out.push({
                  ...
                  title: st.title || "",
                  status: st.status || "",
                  featureTitle: st.feature && st.feature.title ? st.feature.title : "—",
                  projectId: pid,
                  projectTitle: (p.number != null ? "P" + p.number + " - " : "") + (p.name || p.id || "—"),
                  assignee: st.assignee ? ((st.assignee.name && String(st.assignee.name).trim()) ? st.assignee.name.trim() : (st.assignee.email || "Unassigned")) : "Unassigned",
                  sprintName: (st.sprint && st.sprint.name) ? st.sprint.name : "—"
                });
              });
            });
            return out;
        ...
        var flat = [];
        all.forEach(function (arr) { if (Array.isArray(arr)) flat = flat.concat(arr); });
        var dedup = {};
        storiesInProgress = flat.filter(function (st) {
          var key = String(st.id || "") + "|" + String(st.status || "");
          if (!st.id || dedup[key]) return false;
          dedup[key] = true;
          return true;
        }).slice(0, 30);
    ...
    await Promise.all([projectsPromise, summaryPromise, activityPromise, storiesInProgressPromise]);

    var metricCards = [
      {
        title: "Proyectos activos",
        value: summaryAvailable ? String(summary.projectsActive != null ? summary.projectsActive : "—") : "—",
        link: "#/projects",
        linkText: "Ver proyectos",
        icon: "folder",
        tone: "accent",
        optional:
          summaryAvailable && (summary.projectsTotal || 0) > 0 ? "(" + summary.projectsTotal + " en total)" : ""
      },
      {
        title: "Stories",
        value: summaryAvailable ? String(summary.storiesTotal != null ? summary.storiesTotal : "—") : "—",
        link: "#/stories",
      ...
      },
      {
        ...
        value:
          summaryAvailable && summary.activeSprint && summary.activeSprint.progressPercent != null
            ? summary.activeSprint.progressPercent + "%"
            : "—",
        link: "#/sprints",
      ...
      },
      {
        ...
        value: summaryAvailable
          ? String(summary.criticalIncidentsCount != null ? summary.criticalIncidentsCount : "—")
          : "—",
        link: "#/incidents",
    ...
    var cardsHtml = '<div class="dashboard-metrics-grid">';
    metricCards.forEach(function (m) {
      cardsHtml += '<article class="dashboard-metric-card dashboard-metric-' + escapeHtml(m.tone || "accent") + '">';
      cardsHtml += '<div class="dashboard-metric-head">';
      cardsHtml += '<div class="dashboard-metric-title">' + escapeHtml(m.title) + "</div>";
      cardsHtml += '<span class="dashboard-metric-icon" aria-hidden="true"><i data-lucide="' + escapeHtml(m.icon || "circle") + '"></i></span>';
      cardsHtml += "</div>";
      cardsHtml += '<div class="dashboard-metric-value">' + escapeHtml(String(m.value)) + "</div>";
      if (m.optional) {
        cardsHtml += '<div class="dashboard-metric-optional">' + escapeHtml(String(m.optional)) + "</div>";
      }
      if (m.progress !== undefined && m.progress > 0) {
        cardsHtml += '<div class="progress dashboard-metric-progress"><div class="progress-bar" role="progressbar" style="width:' + Number(m.progress) + '%" aria-valuenow="' + m.progress + '" aria-valuemin="0" aria-valuemax="100"></div></div>';
      }
      cardsHtml += '<a href="' + m.link + '" class="dashboard-metric-link">' + escapeHtml(m.linkText) + "</a>";
      cardsHtml += "</article>";
    });
    cardsHtml += "</div>";
    if (!summaryAvailable) {
      cardsHtml +=
        '<div class="alert alert-warning py-2 px-3" role="alert">No se pudo cargar el resumen del dashboard en este momento. Los indicadores pueden mostrarse incompletos.</div>';
    }
    ...
    var projectHealthHtml = buildProjectHealthSection(summary.projectHealth || [], summaryAvailable);
    var recentActivityHtml = buildRecentActivitySection(recentActivity);
    var storiesInProgressHtml = buildStoriesInProgressSection(storiesInProgress);

    var html = "";
    html += '<div class="dashboard-modern">';
    html += breadcrumb + titleHtml;
    html += '<section class="dashboard-section">' + cardsHtml + "</section>";
    html += '<section class="dashboard-grid dashboard-grid-two">';
    html += '<div class="dashboard-col">' + myAssignmentsHtml + "</div>";
    html += '<div class="dashboard-col">' + activeSprintHtml + "</div>";
    html += "</section>";
    html += '<section class="dashboard-grid dashboard-grid-two">';
    html += '<div class="dashboard-col">' + projectHealthHtml + "</div>";
    html += '<div class="dashboard-col">' + recentActivityHtml + "</div>";
    html += "</section>";
    html += '<section class="dashboard-section">' + storiesInProgressHtml + "</section>";
    html += "</div>";
  ...
  function buildMyAssignmentsSection(assignments) {
    var html = '<div class="nexus-panel dashboard-panel">';
    html += '<div class="dashboard-panel-header"><h2 class="dashboard-panel-title">Mis asignaciones</h2><p class="dashboard-panel-subtitle">Tus stories activas y pendientes</p></div>';
    html += '<input type="search" class="form-control form-control-sm mb-3 nexus-input dashboard-assignments-search" placeholder="Buscar por título o ID..." style="max-width:320px" aria-label="Buscar asignaciones">';
    ...
    if (!assignments || assignments.length === 0) {
      ...
      assignments.forEach(function (a) {
        ...
        html += "<td>" + (storyId ? "<button type=\"button\" class=\"btn btn-link p-0 dashboard-story-open-link\" data-story-id=\"" + escapeHtml(storyId) + "\">" + escapeHtml(idDisplay) + "</button>" : escapeHtml(idDisplay)) + "</td>";
        html += "<td>Story</td>";
  ...
  function buildActiveSprintSection(activeSprint) {
    var html = '<div class="nexus-panel dashboard-panel h-100">';
    html += '<div class="dashboard-panel-header"><h2 class="dashboard-panel-title">Estado del sprint activo</h2><p class="dashboard-panel-subtitle">Seguimiento del sprint en curso</p></div>';
    if (!activeSprint || !activeSprint.id) {
      html += '<div class="dashboard-empty-block"><p class="mb-0">No hay sprint activo.</p><a href="#/sprints" class="btn btn-nexus-secondary btn-sm mt-2" aria-label="Ver listado de sprints">Ver sprints</a></div>';
    } else {
      var pct = activeSprint.progressPercent != null ? activeSprint.progressPercent : 0;
      html += '<div class="dashboard-sprint-card">';
      html += '<p class="mb-2"><strong>' + escapeHtml(activeSprint.name || "Sprint activo") + "</strong></p>";
      html += '<div class="progress dashboard-sprint-progress"><div class="progress-bar" role="progressbar" style="width:' + pct + '%" aria-valuenow="' + pct + '" aria-valuemin="0" aria-valuemax="100">' + pct + "%</div></div>";
  ...
  function buildProjectHealthSection(projectHealth, summaryAvailable) {
    var html = '<div class="nexus-panel dashboard-panel h-100">';
    html += '<div class="dashboard-panel-header"><h2 class="dashboard-panel-title">Estado de los proyectos</h2><p class="dashboard-panel-subtitle">Salud y progreso por proyecto</p></div>';
    html += '<div class="table-responsive"><table class="table table-sm nexus-table">';
    html += "<thead><tr><th>Proyecto</th><th>Estado</th><th>Progreso</th><th>Stories</th></tr></thead><tbody>";
    if (!summaryAvailable) {
      html += "<tr><td colspan=\"4\" class=\"text-muted text-center py-4\">Resumen no disponible.</td></tr>";
    } else if (!projectHealth || projectHealth.length === 0) {
      html += "<tr><td colspan=\"4\" class=\"text-muted text-center py-4\">No hay proyectos.</td></tr>";
    } else {
      projectHealth.forEach(function (p) {
        ...
        var progress = Number(p.progressPercent || 0);
        var storyInfo = String(p.storiesDone || 0) + "/" + String(p.storiesTotal || 0);
        var numberPrefix = p.number != null ? "P" + p.number + " - " : "";
        html += "<tr>";
        html += "<td><a href=\"#/projects/" + (p.id || "") + "\">" + escapeHtml(numberPrefix + name) + "</a></td>";
        html += "<td><span class=\"nexus-badge nexus-badge-" + (status ? status.toLowerCase().replace("_", "-") : "archived") + "\">" + escapeHtml(statusLabel) + "</span></td>";
        html += "<td><div class=\"progress\" style=\"height:8px;width:120px\"><div class=\"progress-bar\" style=\"width:" + progress + "%\"></div></div><span class=\"nexus-text-sm nexus-text-muted\">" + progress + "%</span></td>";
        html += "<td><span class=\"nexus-text-sm\">" + escapeHtml(storyInfo) + "</span></td>";
        html += "</tr>";
      });
    }
    html += "</tbody></table></div></div>";
  ...
  function buildRecentActivitySection(auditLogs) {
    var html = '<div class="nexus-panel dashboard-panel h-100">';
    html += '<div class="dashboard-panel-header"><h2 class="dashboard-panel-title">Actividad reciente</h2><p class="dashboard-panel-subtitle">Últimos movimientos del sistema</p></div>';
    html += '<ul class="list-unstyled mb-0 dashboard-activity-list">';
    if (!auditLogs || auditLogs.length === 0) {
      ...
      auditLogs.forEach(function (log) {
        ...
        html += '<li class="dashboard-activity-item"><span class="dashboard-activity-icon" aria-hidden="true"><i data-lucide="history"></i></span><div><span>' + escapeHtml(action) + "</span><br><span class=\"nexus-text-sm nexus-text-muted\">" + escapeHtml(time) + "</span></div></li>";
      });
    }
    html += "</ul></div>";
    return html;
  ...
  function buildStoriesInProgressSection(stories) {
    var html = '<div class="nexus-panel dashboard-panel">';
    html += '<div class="dashboard-panel-header"><h2 class="dashboard-panel-title">Stories in Progress</h2><p class="dashboard-panel-subtitle">Historias activas del equipo (IN_PROGRESS / IN_REVIEW)</p></div>';
    if (!stories || stories.length === 0) {
      html += '<div class="dashboard-empty-block"><p class="mb-0">No hay stories activas en este momento.</p><a href="#/stories" class="btn btn-nexus-secondary btn-sm mt-2">Ver módulo Stories</a></div>';
      html += "</div>";
      return html;
    }
    html += '<div class="table-responsive"><table class="table table-sm nexus-table dashboard-stories-progress-table">';
    html += '<thead><tr><th>Story ID</th><th>Story Title</th><th>Associated Feature</th><th>Project</th><th>Assignee</th><th>Status</th><th>Actions</th></tr></thead><tbody>';
    stories.forEach(function (st) {
      var did = st.number != null ? ("US-" + st.number) : (st.id ? String(st.id).slice(0, 8) : "—");
      var badgeClass = window.nexusBadgeClass ? window.nexusBadgeClass(st.status) : "nexus-badge nexus-badge-in-progress";
      var storyId = st.id || "";
      html += "<tr>";
      html += "<td>" + escapeHtml(did) + "</td>";
      html += "<td>" + escapeHtml(st.title || "—") + "</td>";
      html += "<td>" + escapeHtml(st.featureTitle || "—") + "</td>";
      html += "<td>" + escapeHtml(st.projectTitle || "—") + "</td>";
      html += "<td>" + escapeHtml(st.assignee || "Unassigned") + "</td>";
      html += "<td><span class=\"" + badgeClass + "\">" + escapeHtml(st.status || "—") + "</span></td>";
      html += "<td>";
      if (storyId) {
        html += '<div class="dashboard-story-progress-actions">';
        html += '<button type="button" class="btn btn-link p-0 dashboard-story-progress-view-btn" data-story-id="' + escapeHtml(storyId) + '" aria-label="Ver story"><i data-lucide="eye"></i></button>';
        html += '<button type="button" class="btn btn-link p-0 dashboard-story-progress-edit-btn" data-story-id="' + escapeHtml(storyId) + '" aria-label="Editar story"><i data-lucide="pencil"></i></button>';
        html += "</div>";
      } else {
        html += "—";
      }
  ...
  document.addEventListener("click", function (e) {
    var btn = e.target && e.target.closest && e.target.closest(".dashboard-story-view-btn, .dashboard-story-open-link, .dashboard-story-progress-view-btn, .dashboard-story-progress-edit-btn");
    if (!btn) return;
    e.preventDefault();
    var storyId = btn.getAttribute("data-story-id");
    var startInEdit = btn.classList.contains("dashboard-story-progress-edit-btn");
    if (storyId) openDashboardStoryModal(storyId, startInEdit);
  });

  function openDashboardStoryModal(storyId, startInEdit) {
    if (!storyId) return;
    if (typeof window.openStoryViewModal === "function") {
      window.openStoryViewModal(storyId, { includeStoriesLink: true });
      if (startInEdit) {
        setTimeout(function () {
          var editTab = document.getElementById("story-detail-tab-edicion");
          if (editTab && typeof editTab.click === "function") editTab.click();
        }, 180);
      }
      return;
    }
